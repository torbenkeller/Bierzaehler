...
class CreateNewBeveragePage extends StatefulWidget {
  ...
  @override
  _CreateNewBeveragePageState createState() =>
      _CreateNewBeveragePageState(toUpdate);
}

class _CreateNewBeveragePageState extends State<CreateNewBeveragePage> {
  _CreateNewBeveragePageState(this.toUpdate)
      : _shouldUpdateBeverage = toUpdate != null;
  final Beverage toUpdate;
  ...
  Widget build(BuildContext context) {
    final BeveragesListChangeNotifier bNotifier =
    ...
    return Theme(
      ...
      child: Scaffold(
        body: ChangeNotifierProvider<CategoryListChangeNotifier>(
          create: (BuildContext context) => sl<CategoryListChangeNotifier>(),
          child: Consumer<CategoryListChangeNotifier>(
            builder: (_, CategoryListChangeNotifier cNotifier, __) =>
                CustomScrollView(
              slivers: <Widget>[
                SliverAppBar(
                  actions: <Widget>[
                    IconButton(
                        ...
                        onPressed: () => _submit(cNotifier, bNotifier))
                  ],
                  flexibleSpace: FlexibleSpaceBar(
                    title: Text((_shouldUpdateBeverage)
                        ? 'Getränk ändern'
                        : 'Neues Getränk'),
                          ...
                          Padding(
                            ...
                            child: TextFormField(
                              ...
                              decoration: const InputDecoration(
                                  ...
                                  labelText: 'Alkoholgehalt in % vol.'),
  ...
  List<Widget> _buildCategoriesList(CategoryListChangeNotifier notifier) {
    ...
    return notifier.categoriesList.fold<List<Widget>>(
        (_) => <Widget>[Container()],
        (List<Category> categories) => <Widget>[
  ...
  Future<void> _submit(CategoryListChangeNotifier cNotifier,
      BeveragesListChangeNotifier bNotifier) async {
    if (_beverageFormKey.currentState.validate()) {
      ...
      if (_selectedCategory == 0) {
        ...
        final List<Category> categoriesList = cNotifier.categoriesList
            .fold<List<Category>>(
                (_) => null, (List<Category> categoriesList) => categoriesList);
        if (categoriesList != null) {
          categoryName = categoriesList
              .lastWhere((Category c) => c.id == _selectedCategory)
              .name;
        }
      }
